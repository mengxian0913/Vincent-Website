...
function checkIfHitTop() {
  for (let i = 0; i < section.length - 1; i++) {
    ...
    if (
      ...
      (i == section.length - 1 && Top <= (window.innerHeight * 95) / 100)
    ) {
      nowPosition1.classList.remove("is-active");
      nowPosition2.classList.remove("is-active");
...
function circle() {
  ...
  progress = setInterval(() => {
    for (var i = 0; i < progressStartValue.length; i++) {
      ...
      circularProgress[
        i
      ].style.background = `conic-gradient(${progress_color} ${
        progressStartValue[i] * 3.6
      }deg, #ededed 0deg)`;

      if (
        progressStartValue[i] >= progressEndValue[i] &&
        progressEndValue[i] == MAX_Progress_Value
      ) {
