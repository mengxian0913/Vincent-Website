...
function main() {
  ...
  sideElements.each((index, element) => {
    ...
    const elementTop = sideElement.offset().top;
    const htag = sideElement.parent();
    const htagClassName = htag.attr("class");
    const innerTags = htag.nextUntil(`[class="${htagClassName}"]`);
    let innerFirstTag = null,
      i = 0;
    // console.log(innerTags[0]);
  });
}
...
//     if (innerTags[i].className > htag[0].className)
//       innerFirstTag = innerTags[i].className;
//     i++;
//   }
//
...
//   let isBottom =
//     parseInt(curPos + document.documentElement.clientHeight) ===
...
//           innerTags
//             .filter((index, tag) => tag.className === innerFirstTag)
//             .slideDown();
//         } else {
//           // 收起來
...
//         if (
//           curPos >= top &&
//           curPos < all_htags[index + innerTagsCount].top &&
//           !isBottom
//         )
//           sideElement.attr("style", "color: #337ab7");
